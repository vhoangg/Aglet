    ...
    class admin extends controller
    {
      ...
      function orderDetailProcess()
      {
        if (!strcasecmp($_POST['status'], "confirm")) {
          ...
          $invoice->customQuery($sql);
          echo "thành công";
        } else if (!strcasecmp($_POST['status'], "ship")) {
          $sql = 'UPDATE invoice SET status = 2, ship_at  = "' . $_POST['date'] . '" WHERE id = ' . $_POST['id'];
          $invoice = $this->model("invoiceModel");
          $invoice->customQuery($sql);
          echo "thành công";
        } else if (!strcasecmp($_POST['status'], "arrived")) {
          $sql = 'UPDATE invoice SET status = 3, arrived_at  = "' . $_POST['date'] . '" WHERE id = ' . $_POST['id'];
          $invoice = $this->model("invoiceModel");
          $invoice->customQuery($sql);
          echo "thành công";
        }
